  ...
  try {
    ...
    if (!isValidUserId(userId)) {
      return NextResponse.json(
        { success: false, error: 'Invalid user ID (3-32 alphanumeric chars)' },
        { status: 400 }
      );
    }
    ...
    if (!isValidLanguage(language)) {
      return NextResponse.json(
        { success: false, error: 'Language must be 2-5 uppercase letters (e.g. EN, HI)' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: true, data: { userId, gender, language } },
      { status: 201 }
    );
  } catch (error) {
